import type { InferSelectModel } from 'drizzle-orm';
import {
  varchar,
  timestamp,
  json,
  jsonb,
  uuid,
  text,
  pgSchema,
  real,
  integer,
} from 'drizzle-orm/pg-core';
import type { LanguageModelV2Usage } from '@ai-sdk/provider';
import type { User as SharedUser } from '@chat-template/utils';

const schemaName = 'ai_chatbot';
const customSchema = pgSchema(schemaName);

// Helper function to create table with proper schema handling
// Use the schema object for proper drizzle-kit migration generation
const createTable = customSchema.table;

export const user = createTable('User', {
  id: text('id').primaryKey().notNull(),
  email: varchar('email', { length: 64 }).notNull(),
  // Password removed - using Databricks SSO authentication
});

export type User = SharedUser;

export const chat = createTable('Chat', {
  id: uuid('id').primaryKey().notNull().defaultRandom(),
  createdAt: timestamp('createdAt').notNull(),
  title: text('title').notNull(),
  userId: text('userId').notNull(),
  visibility: varchar('visibility', { enum: ['public', 'private'] })
    .notNull()
    .default('private'),
  lastContext: jsonb('lastContext').$type<LanguageModelV2Usage | null>(),
});

export type Chat = InferSelectModel<typeof chat>;

export const message = createTable('Message', {
  id: uuid('id').primaryKey().notNull().defaultRandom(),
  chatId: uuid('chatId')
    .notNull()
    .references(() => chat.id),
  role: varchar('role').notNull(),
  parts: json('parts').notNull(),
  attachments: json('attachments').notNull(),
  metadata: jsonb('metadata'),
  createdAt: timestamp('createdAt').notNull(),
});

export type DBMessage = InferSelectModel<typeof message>;

export const sentimentAnalysis = createTable('SentimentAnalysis', {
  id: uuid('id').primaryKey().notNull().defaultRandom(),
  userId: text('userId').notNull(),
  headline: text('headline').notNull(),
  company: text('company'),
  status: varchar('status', { enum: ['bullish', 'bearish', 'neutral'] }).notNull(),
  sentimentScore: real('sentimentScore').notNull(),
  confidence: varchar('confidence', { length: 20 }).notNull(),
  confidencePercent: integer('confidencePercent').notNull(),
  rationale: text('rationale'),
  createdAt: timestamp('createdAt').notNull(),
});

export type SentimentAnalysis = InferSelectModel<typeof sentimentAnalysis>;

export const sectorNews = createTable('SectorNews', {
  id: uuid('id').primaryKey().notNull().defaultRandom(),
  sectorId: varchar('sectorId', { length: 50 }).notNull(),
  headline: text('headline').notNull(),
  source: varchar('source', { length: 100 }),
  relatedSymbols: json('relatedSymbols').$type<string[]>(),
  sentimentStatus: varchar('sentimentStatus').notNull(),
  sentimentScore: real('sentimentScore').notNull(),
  confidence: varchar('confidence', { length: 20 }).notNull(),
  confidencePercent: integer('confidencePercent').notNull(),
  impactScore: real('impactScore').notNull().default(0),
  rationale: text('rationale'),
  analyzedAt: timestamp('analyzedAt').notNull(),
});

export type SectorNews = InferSelectModel<typeof sectorNews>;
