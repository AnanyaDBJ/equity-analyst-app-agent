...
const ApprovalStatusBadge = ({ state, approved }: ApprovalStatusBadgeProps) => {
  ...
  // Default: not an MCP approval state
  return null;
};
...
export const McpToolHeader = ({
  ...
  <div className="border-border border-b bg-muted/50">
    ...
    <CollapsibleTrigger
      className={cn(
        'group flex w-full min-w-0 items-center justify-between gap-2 px-3 py-2 transition-colors hover:bg-muted/30',
        className,
      )}
    >
      <div className="flex min-w-0 flex-1 items-center gap-2">
        <span className="truncate font-mono text-sm">{toolName}</span>
        {summary && state === 'output-available' && (
